<?php
     ...
     * rendezést, státusz váltást, jelszó visszaállítást és admin jog kezelést.
     */
...
// Keresés esetén a lapozás visszaállítása az első oldalra
    ...
    /**
     * Felhasználó admin státuszának váltása.
     */
    $toggleAdmin = function ($id) {
        // Biztonsági gát: Csak admin válthat jogot (ha valahogy mégis ide jutna egy nem-admin)
        if (!auth()->user()->is_admin) {
            $this->dispatch('swal:error', message: 'Nincs jogosultságod ehhez!');
            return;
        }

        // Önvédelmi mechanizmus: ne tudd magadat véletlenül kizárni
        if ($id == auth()->id()) {
            $this->dispatch('swal:warning', message: 'Saját magad admin jogosultságát biztonsági okokból nem módosíthatod!');
            return;
        }
        ...
        ActivityLog::create([
            'user_id' => auth()->id(),
            'action' => 'Admin Jog Váltás',
            'description' => "Felhasználó: " . $user->email . " új jogosultsága: " . ($user->is_admin ? 'Admin' : 'Felhasználó'),
        ]);

        $this->dispatch('swal:success', message: 'Jogosultság sikeresen frissítve.');
    };
    ...
    <div class="card-body p-0">
        <table class="table table-hover">
            ...
            <tr>
                ...
                <th>Szerepkör</th> {{-- Új oszlop a láthatóság kedvéért --}}
                <th>Státusz</th>
            ...
            @forelse($this->users as $user)
                <tr wire:key="user-row-{{ $user->id }}">
                    ...
                    <td>
                        {{-- Admin jelvény --}}
                        <span class="badge {{ $user->is_admin ? 'badge-primary' : 'badge-light border' }}">
                    ...
                    <td class="text-right">
                        <div class="btn-group">
                            ...
                            <button wire:click="toggleAdmin({{ $user->id }})"
                                    wire:confirm="Biztosan megváltoztatod {{ $user->name }} jogosultságát?"
                                    wire:loading.attr="disabled"
                                    class="btn btn-default btn-sm"
                                    title="{{ $user->is_admin ? 'Admin jog elvétele' : 'Admin jog adása' }}">
                                <i class="fas fa-shield-alt {{ $user->is_admin ? 'text-primary' : 'text-muted' }}"></i>
                            </button>
            ...
            @empty
                <tr><td colspan="6" class="text-center p-4">Nincs találat.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
